...
export default {
  ...
  methods: {
    ...
    getProvinceData() {
      ...
      this.covData.list.forEach(item => {
        res.push({
          ...
          jwsrNum: item.jwsrNum,      // 境外输入
          value: item.econNum,        // 现有确诊
        })
      })
    ...
    getCityData(provinceName) {
      ...
      // 根据省份的索引获取省份下的所有城市, 以及该城市的确诊人数等数据
    ...
    /* 得到所有地区的ts数据 */
    getTsData() {
      this.tsData = require('../../assets/data/ts.json')
    },
    getCityTsData(provinceName) {

      // 拿到省份的索引
      let index = this.tsData.findIndex(
        (item) => item.name === provinceName
      )

      let cityTsData = []

      this.tsData[index].city.forEach(item => {
        cityTsData.push({
          name: item.name,
          value: item.value
        })
    ...
    initEcharts(pName, Chinese_, state = '0') {
      ...
      if (pName === 'china') {
        ...
        tsSeriesData = this.getProvinceTsData()
        ...
        if (this.currentClick === 'province' || state === '1') {
          ...
          tsSeriesData = this.getCityTsData(arr[index])
