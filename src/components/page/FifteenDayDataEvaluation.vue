...
import { getFifteenDayData } from '@/network/evaluate'

let myChart

export default {
  name: 'FifteenDayDataEvaluation',
  data() {
    return {
      FifteenDayData: {},
      date: '2022-09-16',
  ...
  methods: {
    onSubmit() {
      this.getFifteenDayData(this.date, this.tag)
    ...
    getFifteenDayData(data, tag) {
      getFifteenDayData(data, tag).then(res => {
        if (res !== null) {
          this.FifteenDayData = res
        }
      }).catch(err => {
        return err
      })
    ...
    getLineChartData() {
      let lineChartData = {}, legendData = [], timeInterval = [], typeArray = []
      let isFlag = false

      let data = this.FifteenDayData

      // 拿到从未来15天的日期
      for (let i = 0; i < 15; i++) {
        timeInterval.push(this.getDateAfterNDays(i))
      }
      ...
      for (let prodName in data) {
        ...
        // rain_day、rain_night、temp_high、temp_low、wind_day、wind_night
    ...
    getTableData() {
      ...
      // MJ、MJCN-01、MJCN-02
      for (let prodName in this.FifteenDayData) {
        tableHeaderTop.push(prodName)
        let headerArray = []

        // {rain_day: {}, rain_night: {}, temp_high: {}, ...}
        let prodData = this.FifteenDayData[prodName]
